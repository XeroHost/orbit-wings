...
func (e *Environment) GetLocalIP(ctx context.Context) (string, error) {
	if c, err := e.ContainerInspect(ctx); err != nil {
		...
		if c.State.Running {
			networks := reflect.ValueOf(c.NetworkSettings.Networks).MapKeys()

			if c.NetworkSettings.Networks[networks[0].String()] == nil {
				return "", fmt.Errorf("failedToFetchDockerIp")
			}

			// config.Get().Docker.Network.Name
			return c.NetworkSettings.Networks[networks[0].String()].IPAddress, nil
		}
	}
